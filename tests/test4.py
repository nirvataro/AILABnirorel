...
with xlsxwriter.Workbook('exel files/nqueens_select_2.xlsx') as workbook:
    ...
    for i in range(100):
        ...
        print("choices:", 0, 0, 2)
        iter, time = NQ.main_nqueens(0, 0, 2)
        worksheet_00.write_row(i+1, 0, [iter, time])
        print(i, iter, time)

        print("choices:", 1, 0, 2)
        iter, time = NQ.main_nqueens(1, 0, 2)
        worksheet_10.write_row(i+1, 0, [iter, time])
        print(i, iter, time)

        print("choices:", 0, 1, 2)
        iter, time = NQ.main_nqueens(0, 1, 2)
        worksheet_01.write_row(i+1, 0, [iter, time])
        print(i, iter, time)

        print("choices:", 1, 1, 2)
        iter, time = NQ.main_nqueens(1, 1, 2)
